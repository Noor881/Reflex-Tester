...
const rootPages = {
  ...
  "contact.html": `${head({ title: "Contact ReflexTester", description: "Contact the ReflexTester team.", path: "contact.html" })}<link rel="stylesheet" href="./src/styles/article.css"></head><body><main id="main" class="article-layout"><article class="article"><span class="eyebrow">Contact</span><h1>Report a problem or suggest an improvement.</h1><p>Include the tool name, browser, device type and the steps that produced the issue.</p><form class="contact-form" data-contact-form><label class="field"><span>Name</span><input name="name" required maxlength="100" autocomplete="name"></label><label class="field"><span>Email</span><input name="email" type="email" required maxlength="200" autocomplete="email"></label><label class="field"><span>Subject</span><input name="subject" maxlength="180"></label><label class="field"><span>Message</span><textarea name="message" required minlength="10" maxlength="4000" rows="8"></textarea></label><label class="contact-trap" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label><button class="button accent" type="submit">Send message</button><p class="form-status" data-contact-status role="status" aria-live="polite"></p></form><p class="muted">Prefer email? <a href="mailto:contact@reflextester.fun">contact@reflextester.fun</a></p></article></main><script type="module" src="./src/app/contact.js"></script><script type="module">import{mountShell}from'./src/app/shell.js';mountShell()</script></body></html>`,
...
const renderAffiliate = (item) =>
  `<article class="article"><span class="eyebrow">${esc(item.category)}</span><h1>${esc(item.title)}</h1><div class="article-meta">Independent buyer’s guide · Updated ${dateLabel(item.updatedAt)} · 5 min read</div><p class="affiliate-disclosure"><strong>Affiliate disclosure:</strong> As an Amazon Associate, ReflexTester may earn from qualifying purchases made through links on this page, at no additional cost to you.</p><p class="article-deck">${esc(item.description)}</p>${section("product-summary", "Product summary", `<div class="affiliate-product"><h3>${esc(item.product)}</h3><p>${esc(item.summary)}</p><a class="affiliate-button" data-affiliate-click="${esc(item.slug)}" href="${item.link}" target="_blank" rel="sponsored nofollow noopener">Check the current Amazon UK listing →</a><p><small>Price, seller, availability and listing details can change. Confirm them before ordering.</small></p></div>`)}${section("before-buying", "What to check before buying", list(item.checks))}${section("fit", "Who it may suit", `<p>${esc(item.verdict)}</p><p>This page is an editorial overview, not a substitute for the current product label, manufacturer instructions or professional advice.</p>`)}${item.body ? renderMarkdown(item.body) : ""}${section("decision", "A sensible buying decision", "<p>Open the current listing, verify the exact product and pack, compare the total delivered price and read recent reviews for recurring issues. Avoid relying on a single rating or promotional claim.</p>")}<div class="affiliate-product"><h2 id="current-listing">Check the current offer</h2><a class="affiliate-button" data-affiliate-click="${esc(item.slug)}" href="${item.link}" target="_blank" rel="sponsored nofollow noopener">View on Amazon UK →</a></div></article>`;
