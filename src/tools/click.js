...
export function mountClick(root, tool) {
  ...
  root.querySelectorAll("[data-seconds]").forEach(
    (button) =>
      (button.onclick = () => {
        if (running) return;
  ...
  function finish() {
    ...
    const score = clicks / duration;
    const attempts = store.record(tool.slug, {
      score,
      clicks,
      duration,
      unit: "CPS",
    });
    const stats = summarize(attempts, "score");
    value.innerHTML = `${score.toFixed(1)}<span class="instrument-unit">CPS</span>`;
    status.textContent = `${clicks} clicks · personal best ${stats.best.toFixed(1)} CPS`;
    zone.textContent = "Try again";
  }
  function tick() {
    const elapsed = (performance.now() - start) / 1000;
    if (elapsed >= duration) return finish();
    value.innerHTML = `${(clicks / Math.max(elapsed, 0.01)).toFixed(1)}<span class="instrument-unit">CPS</span>`;
    status.textContent = `${(duration - elapsed).toFixed(1)} seconds remaining`;
