...
const modes = {
  grid: {
    ...
    size: 56,
    label:
      "Three targets remain active. Clear them as efficiently as possible.",
  },
  spider: {
    seconds: 30,
    targets: 1,
    size: 52,
    label: "Return to the centre anchor between every outside target.",
  },
  flick: {
    seconds: 25,
    targets: 1,
    size: 46,
    label: "Acquire each target from a fresh centre starting position.",
  },
  precision: {
    seconds: 30,
    targets: 1,
    size: 38,
    label: "Targets shrink as your hit streak increases.",
  },
  valorant: {
    seconds: 30,
    targets: 1,
    size: 34,
    label: "Targets appear around a narrow head-height band.",
  },
  cs2: {
    seconds: 30,
    targets: 2,
    size: 36,
    label: "Clear paired targets with controlled, accurate taps.",
  },
  cod: {
    seconds: 25,
    targets: 2,
    size: 50,
    lifetime: 900,
    label: "Targets disappear quickly. Prioritise fast acquisition.",
  },
  fortnite: {
    seconds: 30,
    targets: 1,
    size: 72,
    lifetime: 1500,
    label: "Track large close-range targets across changing elevations.",
  },
};
...
function finish(root, tool, stats) {
  ...
  const score =
    tool.mechanic === "tracking" || tool.mechanic === "apex"
      ? Math.round(stats.tracked)
      : stats.hits;
  store.record(tool.slug, {
    ...
    shots: stats.shots,
    unit:
      tool.mechanic.includes("tracking") || tool.mechanic === "apex"
        ? "points"
        : "hits",
  });
    ...
    const frame = (now) => {
      ...
      if (remaining <= 0) {
        ...
        const contact = Math.max(
          0,
          Math.min(100, ((stats.tracked * 10) / total) * 100),
        );
        stats.shots = 100;
        stats.hits = contact;
      ...
      angle += elapsed * 0.0011 * speed;
      const x = rect.width * 0.5 + Math.sin(angle * 1.17) * rect.width * 0.34;
      const y =
        rect.height * 0.52 + Math.sin(angle * 1.83) * rect.height * 0.29;
      ...
      stage.querySelector("[data-score]").textContent = Math.round(
        stats.tracked,
      );
      stage.querySelector("[data-contact]").textContent = Math.round(
        ((stats.tracked * 10) / Math.max(1, total)) * 100,
      );
...
function mountEdit(root, tool) {
  ...
  game.querySelector("button").onclick = () => {
    ...
    const grid = game.querySelector(".edit-grid"),
      reference = game.querySelector(".edit-reference");
    for (let i = 0; i < 9; i++) {
      grid.insertAdjacentHTML(
        "beforeend",
        `<button data-cell="${i}" aria-label="Edit tile ${i + 1}"></button>`,
      );
      reference.insertAdjacentHTML("beforeend", `<i data-ref="${i}"></i>`);
    }
    const next = () => {
      selected.clear();
      goal = patterns[Math.floor(Math.random() * patterns.length)];
      grid
        .querySelectorAll("button")
        .forEach((x) => x.classList.remove("selected"));
      reference
        .querySelectorAll("i")
        .forEach((x, i) => x.classList.toggle("active", goal.includes(i)));
    };
    grid.onclick = (event) => {
      const tile = event.target.closest("button");
      if (!tile) return;
      const value = Number(tile.dataset.cell);
      selected.has(value) ? selected.delete(value) : selected.add(value);
      tile.classList.toggle("selected");
    };
    game.querySelector("[data-confirm]").onclick = () => {
      stats.shots++;
      const correct =
        goal.length === selected.size && goal.every((x) => selected.has(x));
      if (correct) {
        stats.hits++;
        stats.streak++;
      ...
      game.querySelector("[data-score]").textContent = stats.hits;
      game.querySelector("[data-streak]").textContent = stats.streak;
      next();
    };
    next();
    const frame = (now) => {
      const remaining = (endAt - now) / 1000;
      if (remaining <= 0) return finish(root, tool, stats);
      game.querySelector("[data-time]").textContent = remaining.toFixed(1);
      requestAnimationFrame(frame);
    };
    requestAnimationFrame(frame);
  };
...
function mountTargets(root, tool) {
  ...
  let running = false,
    endAt = 0,
    targetId = 0,
    centreRequired = tool.mechanic === "spider" || tool.mechanic === "flick";
  ...
  const stage = root.firstElementChild;
  const update = () => {
    stage.querySelector("[data-score]").textContent = stats.hits;
    stage.querySelector("[data-accuracy]").textContent = Math.round(
      (stats.hits / Math.max(1, stats.shots)) * 100,
    );
    stage.querySelector("[data-streak]").textContent = stats.streak;
  };
  const place = (target) => {
    const rect = stage.getBoundingClientRect(),
      pad = 55;
    let x = pad + Math.random() * Math.max(1, rect.width - pad * 2),
      y = pad + Math.random() * Math.max(1, rect.height - pad * 2);
    if (["valorant", "cs2"].includes(tool.mechanic))
      y = rect.height * 0.36 + (Math.random() - 0.5) * 54;
    if (tool.mechanic === "fortnite")
      y = rect.height * (0.25 + Math.random() * 0.58);
    target.style.left = `${x}px`;
    target.style.top = `${y}px`;
  };
  const createTarget = (isCentre = false) => {
    const target = document.createElement("button");
    target.className = `aim-target ${["precision", "valorant", "cs2"].includes(tool.mechanic) ? "small" : ""} ${isCentre ? "anchor" : ""}`;
    target.style.width =
      target.style.height = `${tool.mechanic === "precision" ? Math.max(20, mode.size - stats.streak * 2) : mode.size}px`;
    target.setAttribute("aria-label", isCentre ? "Centre anchor" : "Target");
    if (isCentre) {
    ...
    target.onclick = (event) => {
      ...
      target.remove();
      update();
      if (centreRequired && !isCentre) createTarget(true);
      else if (centreRequired && isCentre) createTarget(false);
      else replenish();
    };
    stage.append(target);
    if (mode.lifetime && !isCentre)
      setTimeout(() => {
        if (running && target.isConnected && id <= targetId) {
          target.remove();
          stats.misses++;
          stats.streak = 0;
          update();
  ...
  const replenish = () => {
    while (
      running &&
      stage.querySelectorAll(".aim-target").length < mode.targets
    )
      createTarget(false);
  };
  stage.onclick = () => {
  ...
  stage.querySelector("button").onclick = (event) => {
    event.stopPropagation();
    running = true;
    endAt = performance.now() + mode.seconds * 1000;
    stage.innerHTML = `<div class="aim-hud"><span><strong data-time>${mode.seconds.toFixed(1)}</strong>s</span><span><strong data-score>0</strong> hits</span><span><strong data-accuracy>100</strong>% accuracy</span><span><strong data-streak>0</strong> streak</span></div>`;
    centreRequired ? createTarget(true) : replenish();
    const frame = (now) => {
      if (!running) return;
      const remaining = (endAt - now) / 1000;
      if (remaining <= 0) {
...
export function mountAim(root, tool) {
  if (tool.mechanic === "tracking" || tool.mechanic === "apex")
    return mountTracking(root, tool);
