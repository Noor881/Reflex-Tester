...
function resultsView(root, tool, values, unit = "ms", lower = true) {
  ...
  root
    ...
    .addEventListener("click", () => mountReaction(root, tool));
}
...
async function reactionRound(root, audio, attempt) {
  return new Promise((resolve) => {
    ...
    const activate = async () => {
      ...
      field.className = "reaction-field go";
    ...
    const respond = () => {
      if (state === "waiting") {
        state = "early";
        field.className = "reaction-field early";
        field.textContent = "Too early — this attempt will restart";
      ...
      state = "done";
      field.textContent = `${Math.round(score)} ms`;
      setTimeout(() => resolve(score), 450);
    };
...
async function reactionSession(root, tool, audio = false) {
  ...
  while (samples.length < 5) {
    const score = await reactionRound(root, audio, samples.length + 1);
  ...
  resultsView(
    root,
    tool,
    store.record(tool.slug, { score, samples, unit: "ms" }),
  );
}
...
function doubleClick(root, tool) {
  ...
  field.addEventListener("pointerdown", () => {
    ...
    if (!first) {
      first = now;
      field.textContent = "Click again";
      return;
    }
    const score = now - first;
    if (score > 900) {
      first = now;
      field.textContent = "Too slow. First click recorded again.";
      return;
    }
